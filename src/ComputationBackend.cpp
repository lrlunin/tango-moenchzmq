...
void ComputationBackend::thread_task(){
        ...
        while (true){
            while (!sleep && frame_ptr_queue.pop(ff_ptr)){
                ...
                if (pedestal){
                    for (auto i = 0; i < 16000; ++i){
                        if (pedestal_counter[i] < pedestal_buff_size) {
                            pedestal_counter[i]++;
                            pedestal_frame[i] = pedestal_frame[i] + ff_ptr->arr[i];
                        }
                        else {
                            pedestal_frame[i] = pedestal_frame[i] + ff_ptr->arr[i] - pedestal_frame[i]/pedestal_counter[i];
                        }
                    }
                    int pedestal[16000];
                    calcPedestal(pedestal);
                }
