...
protected:
    ...
    FileWriter(int file_index, std::string file_name, std::filesystem::path root_path):file_index(file_index), file_name(file_name), root_path(root_path){};
    ...
    virtual std::string buildFullFilePath() = 0;
public:
    virtual ~FileWriter() {};
    virtual void writeFrame(const std::string frame_name, OrderedFrame<float, consts::LENGTH> &frame) = 0;
};
